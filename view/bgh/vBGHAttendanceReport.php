...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-info { background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); color: white; font-size: 12px; padding: 6px 12px; }
        ...
        .stat-icon { width: 60px; height: 60px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 26px; }
        ...
        .main-content { background: white; padding: 30px; border-radius: 15px; box-shadow: 0 8px 20px rgba(0, 0, 0, 0.15); }
        ...
        .filter-header { display: flex; align-items: center; gap: 10px; margin-bottom: 20px; color: #374151; font-weight: 600; font-size: 16px; }
        .filter-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 15px; }
        .form-group { display: flex; flex-direction: column; }
        .form-group label { color: #374151; margin-bottom: 8px; font-weight: 600; font-size: 13px; text-transform: uppercase; letter-spacing: 0.5px; }
        .form-group select { padding: 12px 15px; border: 2px solid #e5e7eb; border-radius: 8px; font-size: 14px; transition: all 0.3s; background: white; }
        ...
        table.dataTable thead th { padding: 15px 12px; text-align: center; border: none; color: white; font-weight: 600; font-size: 13px; text-transform: uppercase; letter-spacing: 0.5px; }
        table.dataTable tbody td { padding: 12px; text-align: center; border-bottom: 1px solid #e5e7eb; font-size: 13px; vertical-align: middle; }
        table.dataTable tbody tr:hover { background-color: #f3f4f6; }
        .attendance-badge { display: inline-block; padding: 6px 14px; border-radius: 20px; font-weight: 600; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px; }
        .attendance-tot { background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; }
        .attendance-kha { background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); color: white; }
        .attendance-trung-binh { background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); color: white; }
        .attendance-yeu { background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); color: white; }
        .no-data { text-align: center; padding: 50px; color: #6b7280; font-style: italic; }
        .modal { display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.5); }
        .modal-content { background-color: white; margin: 5% auto; padding: 0; border-radius: 15px; width: 80%; max-width: 800px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); }
        ...
        .close { color: white; font-size: 32px; font-weight: bold; cursor: pointer; transition: all 0.3s; }
        ...
        .detail-table th { background: #f9fafb; font-weight: 600; color: #374151; }
        .detail-table tr:hover { background: #f9fafb; }
        .type-badge { padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: 600; }
        .type-cophep { background: #d1fae5; color: #065f46; }
        .type-khongphep { background: #fee2e2; color: #991b1b; }
        ...
        .dataTables_wrapper .dataTables_paginate .paginate_button.current { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white !important; border-color: #667eea; }
        @media (max-width: 768px) { .stats-grid, .filter-grid { grid-template-columns: 1fr; } .header { flex-direction: column; gap: 15px; } .modal-content { width: 95%; margin: 10% auto; } }
    </style>
</head>
<body>
    <?php
    $tongHocSinh = 0; $tongChuyenCanTot = 0; $tongChuyenCanKha = 0; $tongChuyenCanYeu = 0; $tongNghiCoPhep = 0; $tongNghiKhongPhep = 0;
    if (!empty($duLieuBaoCao)) {
        $tongHocSinh = count($duLieuBaoCao);
        foreach ($duLieuBaoCao as $row) {
            $tongNghiCoPhep += $row['soNghiCoPhep']; $tongNghiKhongPhep += $row['soNghiKhongPhep'];
            if (isset($row['xepLoaiChuyenCan'])) {
                switch ($row['xepLoaiChuyenCan']) {
                    case 'Tốt': $tongChuyenCanTot++; break;
                    case 'Khá': $tongChuyenCanKha++; break;
                    default: $tongChuyenCanYeu++; break;
                }
            }
        }
    }
    ...
    <div class="container">
        ...
        <div class="stats-grid">
            <div class="stat-card"><div class="stat-icon blue"><i class="fas fa-users"></i></div><div class="stat-details"><h3>Tổng số học sinh</h3><div class="number"><?php echo number_format($tongHocSinh); ?></div></div></div>
            <div class="stat-card"><div class="stat-icon green"><i class="fas fa-check-circle"></i></div><div class="stat-details"><h3>Chuyên cần tốt</h3><div class="number"><?php echo number_format($tongChuyenCanTot); ?></div></div></div>
            <div class="stat-card"><div class="stat-icon orange"><i class="fas fa-calendar-check"></i></div><div class="stat-details"><h3>Nghỉ có phép</h3><div class="number"><?php echo number_format($tongNghiCoPhep); ?></div></div></div>
            <div class="stat-card"><div class="stat-icon red"><i class="fas fa-exclamation-triangle"></i></div><div class="stat-details"><h3>Nghỉ không phép</h3><div class="number"><?php echo number_format($tongNghiKhongPhep); ?></div></div></div>
        </div>
        <div class="main-content">
            ...
            <div class="table-container">
                <table id="attendanceTable" class="display responsive nowrap" style="width:100%">
                    <thead>
                        <tr><th>STT</th><th>Mã HS</th><th>Họ tên</th><th>Lớp</th><th>Khối</th><th>Tổng nghỉ</th><th>Có phép</th><th>Không phép</th><th>Xếp loại</th><th>GVCN</th><th>Chi tiết</th></tr>
                    </thead>
                    <tbody>
                        <?php if (empty($duLieuBaoCao)): ?>
                            <tr><td colspan="11" class="no-data"><i class="fas fa-inbox"></i><br>Không có dữ liệu để hiển thị.<br><small>Vui lòng chọn bộ lọc phù hợp.</small></td></tr>
                        <?php else: $stt = 1; foreach ($duLieuBaoCao as $row): 
                            $badgeClass = 'attendance-badge';
                            switch ($row['xepLoaiChuyenCan']) {
                                case 'Tốt': $badgeClass .= ' attendance-tot'; break;
                                case 'Khá': $badgeClass .= ' attendance-kha'; break;
                                case 'Trung bình': $badgeClass .= ' attendance-trung-binh'; break;
                                default: $badgeClass .= ' attendance-yeu'; break;
                            }
                        ?>
                            <tr>
                                <td><?php echo $stt++; ?></td>
    ...
    <div id="detailModal" class="modal">
        <div class="modal-content">
            <div class="modal-header"><h2><i class="fas fa-info-circle"></i> Chi tiết ngày nghỉ - <span id="modalStudentName"></span></h2><span class="close">&times;</span></div>
            <div class="modal-body"><div id="modalContent"><div style="text-align: center; padding: 40px;"><i class="fas fa-spinner fa-spin" style="font-size: 32px; color: #667eea;"></i><p style="margin-top: 15px; color: #6b7280;">Đang tải dữ liệu...</p></div></div></div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#attendanceTable').DataTable({
                responsive: true, pageLength: 25, lengthMenu: [[10, 25, 50, 100, -1], [10, 25, 50, 100, "Tất cả"]],
                language: { "sProcessing": "Đang xử lý...", "sLengthMenu": "Hiển thị _MENU_ bản ghi", "sZeroRecords": "Không tìm thấy dữ liệu", "sInfo": "Hiển thị _START_ đến _END_ trong tổng số _TOTAL_ bản ghi", "sInfoEmpty": "Hiển thị 0 đến 0 trong tổng số 0 bản ghi", "sInfoFiltered": "(lọc từ _MAX_ bản ghi)", "sSearch": "Tìm kiếm:", "oPaginate": { "sFirst": "Đầu", "sPrevious": "Trước", "sNext": "Tiếp", "sLast": "Cuối" } },
                order: [[5, 'desc']], columnDefs: [{ orderable: false, targets: [10] }]
            });
            <?php endif; ?>
            var modal = document.getElementById("detailModal");
            var span = document.getElementsByClassName("close")[0];
            $('.view-detail-btn').click(function() {
                var maHS = $(this).data('mahs'); var tenHS = $(this).data('tenhs'); var hocKy = $(this).data('hocky'); var namHoc = $(this).data('namhoc');
                $('#modalStudentName').text(tenHS); modal.style.display = "block";
                $.ajax({ url: 'cBGHReport.php', method: 'GET', data: { action: 'chi-tiet-nghi-hoc', maHS: maHS, hocKy: hocKy, namHoc: namHoc },
                    success: function(response) { $('#modalContent').html(response); },
                    error: function() { $('#modalContent').html('<p style="color: red; text-align: center;">Có lỗi xảy ra khi tải dữ liệu!</p>'); }
                });
            });
